(function () {
    ...
    if (!assessmentError && Array.isArray(assessments)) {
      ...
      assessments.forEach((row) => {
        ...
        map.set(externalId, {
          ...
          gripAssessment: row.grip_assessment || null,
  ...
  async function syncAssessmentToCloud(assessment) {
    ...
    const { data, error } = await client.from('assessments').upsert({
      ...
      grip_assessment: assessment.gripAssessment || null,
    ...
    if (error) { console.error('[sync-assessment] upsert failed:', error.message, error, { externalId: assessment.externalId, userId: assessment.userId }); return; }
  ...
  function saveAssessment(payload) {
    ...
    const assessment = {
      ...
      gripAssessment: payload.gripAssessment || null,
