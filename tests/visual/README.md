...
Committed PNGs live at `tests/visual/visual.spec.js-snapshots/`.
Filenames encode the test name + project (desktop/mobile) + platform
(win32 / linux / darwin). Font rendering differs enough between platforms
that each needs its own baselines — Playwright writes them automatically
the first time a run on that platform finds its file missing.

**First-time set committed here (win32):** generated by me running the
suite locally on Windows. Linux baselines will be auto-written on the
first CI run; macOS baselines only matter if someone runs the suite on
a Mac and CI also runs there (currently CI is Ubuntu-only).
